...
def read_snapshots(path: str):
    ...
    with open(path, newline="", encoding="utf-8") as f:
        ...
        required = {"run", "hop", "x_start", "x_end", "node_id", "parent_id", "time", "pop", "inversion"}
        if not reader.fieldnames or not required.issubset(set(reader.fieldnames)):
            missing = required - set(reader.fieldnames or [])
            raise ValueError(f"snapshot CSV missing columns: {sorted(missing)}")
        for row in reader:
            key = (
                int(row["run"]),
                int(row["hop"]),
            ...
            rows_by_snapshot[key].append(row)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Render one SMC tree from smc_tree_snapshots.csv.")
    parser.add_argument("--snapshot-csv", default="smc_tree_snapshots.csv")
    ...
    if not os.path.isfile(args.snapshot_csv):
        print(f"Error: file not found: {args.snapshot_csv}", file=sys.stderr)
        return 1

    try:
        snapshots = read_snapshots(args.snapshot_csv)
